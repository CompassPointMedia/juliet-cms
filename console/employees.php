...
$navObject='Employees_ID';
$updateMode='updateEmployee';
$insertMode='insertEmployee';
$deleteMode='deleteEmployee';
...
$ids=q("SELECT * FROM finan_employees", O_COL);
$nullCount=count($ids);
$j=0;
if($nullCount){
	foreach($ids as $v){
...
if(strlen($$object)){
	...
	if($a=q("SELECT * FROM finan_employees WHERE ID='".$$object."'", O_ROW)){
		$mode=$updateMode;
		@extract($a);
	}else{
...
<div id="header"><!-- InstanceBeginEditable name="top_nav" -->
	<div id="headerBar1" style="padding:5px 10px 10px 12px;">
		...
		echo ($mode==$insertMode ? 'Create a new ' : 'Edit ') . 'Employee Entry';
		?></h2>
	</div>
	<div class="cb">&nbsp;</div>
...
<?php
require('components/comp_60_employeeform_v100.php');
?> 
