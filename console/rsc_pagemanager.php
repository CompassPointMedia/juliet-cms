...
<h1>Page Manager</h1>
<a href="rsc_menu_gui.php" target="_blank">Menu Visual Editor</a><br />
<style type="text/css">
.spacer td{
  ...
  <tbody>
	...
	if($pages=q("SELECT 
		...
		foreach($pages as $page){
			if(!$havenav){
				$havenav=true;
				if($nav=q("SELECT v.*, n.Name AS MenuName FROM _v_gen_nodes_hierarchy_nav v, gen_nodes n WHERE n.ID=v.GroupNodes_ID ORDER BY GroupNodes_ID", O_ARRAY_ASSOC)){
			...
			?><tr id="r_<?php echo $page['ID'];?>" class="<?php echo !$page['Active']?'inactive':''?>">
			  ...
			  <td><?php if($page['Active'])echo 'Y';?></td>
			  <td nowrap="nowrap"><a href="http://<?php echo $HTTP_HOST?>/<?php echo str_replace(' ','-',strtolower($linkNodeName))?>" target="_blank"><?php echo $visibleNodeName?></a><?php
			  ?></td>
