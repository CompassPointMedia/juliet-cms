...
$datasetFieldList='a.*, c.ID AS Clients_ID'.($useStatusFilterOptions ? ', c.Statuses_ID':'');
...
if(!$refreshComponentOnly){
	...
	function addMember(){
		ow('members.php?cbFunction=refreshList','l1_members','700,700',true);
		return false;
	}
	function openMember(){
		for(var j in hl_grp['memopt'])j=j.replace('r_','');
		ow('members.php?Clients_ID='+j,'l1_members','700,700');
	}
...
<div id="<?php echo $datasetComponent?>" refreshparams="noparams">
	...
	<table width="100%" border="0" cellspacing="0" cellpadding="0" class="data1" style="clear:both;">
		...
		<tbody <?php if($browser=='Moz')echo 'style="overflow-y:scroll;height:350px;"';?>>
		<?php
		if($records)
