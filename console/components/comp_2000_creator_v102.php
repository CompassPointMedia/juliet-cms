...
2012-01-27:forked off completely new logic and dataobjects stored now in a database (relatebase_rfm.
see comp_1000_creator_v101.php for previous notes
*/
...
//should we be in a form - normally we are 

//get the dataset coding
$authorized=true;
$editing=true;
...
/*
todo
	get datasetHex in all cases
	
*/
if($dataset){
	if($a=q("SELECT * FROM system_dataobjects WHERE dataset='$dataset'", O_ROW, C_SUPER)){
		@extract($a);
		@extract(unserialize(base64_decode($a['datasetHex'])));
	}else{
		q("INSERT INTO system_dataobjects SET dataset='$dataset', CreateDate=NOW(), Creator='".($_SESSION['admin']['userName'] ? $_SESSION['admin']['userName'] : sun())."'");
	}
	$presentation='database';
}else{
	//refresh component coding
	$qs=preg_replace('/^&*Location=[^&]*/','',$QUERY_STRING);
	$qs.=($qs?'&':'').'Location='.md5(time().rand(1,1000000));

	if(preg_match('/^[a-f0-9]{32}$/',$Location)){
		if($a=$_SESSION['special']['datasets'][$Location]){
			extract($a);
		}else{
			//redirect
			mail($developerEmail, 'Error in '.$MASTER_USERNAME.':'.end(explode('/',__FILE__)).', line '.__LINE__,get_globals('redirect dataobject creator'),$fromHdrBugs);
			$_SESSION['special']['datasets'][$Location]=array( 'starttime'=>time(),	);
			header('Location: '. $thispage.'?'.$qs);
			exit;
		}
		$presentation='session';
	}else if(strlen($Location)){
		if(!(@$a=file($Location)))exit('unable to pull specified file location');
		...
		if(false){
			...
			mail($developerEmail, 'Error in '.$MASTER_USERNAME.':'.end(explode('/',__FILE__)).', line '.__LINE__,get_globals($err='reading existing dataobjects files (for conversion to the database) is undeveloped'),$fromHdrBugs);
...
if($editing && $authorized){
	...
	if($modePassed){
		...
		if($submodePassed=='insertColumnBelow'){
			break;
		}
		if($submodePassed=='deleteColumn'){
			break;
		}
		if($submodePassed=='loadNewCols'){
	...
	<input type="hidden" name="modePassed" id="modePassed" value="<?php echo $modePassed?>" />
	<input type="hidden" name="submodePassed" id="submodePassed" value="" />
	<input type="hidden" name="presentation" id="presentation" value="<?php echo $presentation;?>" />
	...
	<p>
	Dataset Handle : <input name="dataset" type="text" id="dataset" value="<?php echo $dataset;?>" /> 
	(this must be unique)<br />
	Dataset Group: -not developed-
	<br />
	...
	if($a=$availableCols[$datasetGroup][$modApType][$modApHandle]['scheme']){
	  ...
	  <div id="col_<?php echo $i?>" class="datasetCol">
		  <div class="datasetColTools">
			  ...
			  &nbsp;&nbsp;
			  <a href="#" onclick="return manageCols(this,<?php echo $i?>,'delete');" title="delete this column"><div style="float:left;"><img src="/images/i/antset.delete.png" width="28" height="28" /></div></a>		  </div>
		  Column name: 
