		...
		foreach($a as $n=>$v){
			...
			if($fctn=$replaceExtendOption[$v['ID']]){
				...
				*/
				...
				?><h3 class="nullTop nullBottom"><a onclick="return ow(this.href,'l1_movenodes','700,600');" href="rsc_menu_gui_focus.php?ID=<?php echo $v['ID'];?>"><?php
				if($v['NameT4']){
					echo $v['NameT4'];
				}else{
					?><span class="pageOnly" title="this is the page name (no node name)">*<?php echo $v['PageName'];?></span><?php
				}
				?></a></h3><?php
				if($v['ParentNodes_ID']){
