...
$datasetTable='addr_contacts';
$datasetActiveUsage=true;
$datasetFieldList='a.*, c.ID AS Clients_ID, d.Name AS StatusName, d.ColorCode, d.StatusHandle'.($useStatusFilterOptions ? ', c.Statuses_ID':'');
...
switch(true){
	case $sort=='address':
		...
		LEFT JOIN finan_ClientsContacts cc ON a.ID=cc.Contacts_ID AND cc.Type='Primary'
		LEFT JOIN finan_clients c ON cc.Clients_ID=c.ID
		LEFT JOIN finan_clients_statuses d ON c.Statuses_ID=d.ID
		WHERE c.ResourceType IS NOT NULL ".
		...
		"$filterQueries
		GROUP BY a.ID
		ORDER BY a.HomeState $asc, a.HomeCity $asc, a.HomeAddress $asc $limit", O_ARRAY_ASSOC);
	break;
	default: //Name
		$records=q("SELECT
		$datasetFieldList 
		...
		"$filterQueries
		GROUP BY a.ID
		ORDER BY a.LastName $asc, a.FirstName $asc $limit", O_ARRAY_ASSOC);
}
$recordCols=$qr['cols'];
...
<div id="<?php echo $datasetComponent?>" refreshparams="noparams">
	...
	<table width="100%" border="0" cellspacing="0" cellpadding="0" class="data1" style="clear:both;">
		...
		<tbody id="<?php echo $datasetComponent?>_tbody" <?php if($browser=='Moz')echo 'style="overflow-y:scroll;overflow-x:none;height:350px;"';?>>
		<?php
		$datasetOutput='';
		if($records)
		foreach($records as $v){
			...
			$i++;
			//get permissions
			$access=q("SELECT b.Name, 'Y' FROM addr_ContactsAccess a, addr_access b WHERE a.Access_ID=b.ID AND a.Contacts_ID=".$v['ID'], O_COL_ASSOC);
			extract($v);
			$deletable=($r=q("SELECT COUNT(*) FROM finan_invoices WHERE Clients_ID=$Clients_ID", O_VALUE) ? false : true);
			...
			?><tr id="r_<?php echo $Clients_ID?>" onclick="h(this,'memopt',0,0,event);" ondblclick="h(this,'memopt',0,0,event);open<?php echo $dataset?>();" oncontextmenu="h(this,'memopt',0,1,event);" <?php if(!fmod($i,2))echo 'class="alt"';?>  deletable="<?php echo $deletable?>">
				<?php if(!$userSettings['hideInactive'.$dataset]){ ?>
				<td id="r_<?php echo $Clients_ID?>_active" title="Make this <?php echo $dataset?> <?php echo $Active ? 'in':''?>active" onclick="toggleActiveObject(<?php echo $Clients_ID?>);" active="<?php echo $Active?>"><?php
				if(!$Active){
				...
				if($deletable){
					?><a title="Delete this member" href="resources/bais_01_exe.php?mode=deleteClient&amp;Clients_ID=<?php echo $Clients_ID?>&amp;cb=refreshList" target="w2" onClick="if(!confirm('This will permanently delete this member\'s record.  Are you sure?'))return false;">&nbsp;<img src="../images/i/del2.gif" alt="delete" width="16" height="18" border="0" /></a><?php
				}else{
					?>&nbsp;<img src="../images/assets/spacer.gif" width="18" height="18" /><?php
				}
				?>&nbsp;&nbsp;<a title="Edit this member's information" href="members.php?Clients_ID=<?php echo $Clients_ID?>" onClick="return ow(this.href,'l1_members','700,700');return false;"><img src="../images/i/edit2.gif" width="15" height="18" border="0"></a>&nbsp;</td>
				<!-- user columns -->
