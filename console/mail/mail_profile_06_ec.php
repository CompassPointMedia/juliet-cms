...
switch($RecipientMethod){
	...
	case 'import':
		...
		****/
		...
		$fileFormatOK=1;
		switch(true){
			case ($compileTime>@filemtime($VOS_ROOT.'/'.$acct.'/tmp_mailprofile'.$Profiles_ID.'.txt')): //the file is not present, or it has not been uploaded since the window was open.  This means if they close the profile, they must upload again
				errLevel(1);
...
function valid_columns($mode){
	global $acct, $Profiles_ID, $qual, $_emailColumns, $VOS_ROOT;
	//check email columns
