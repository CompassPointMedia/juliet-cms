...
<p><a href="mail.php" title="Open a new mail profile window and send emails" onclick="return ow(this.href,'l1_mail','800,600',true);">Launch New Mail Tool</a> </p>
<?php
if($profiles=q("SELECT * FROM relatebase_mail_profiles p WHERE ResourceType IS NOT NULL", O_ARRAY)){
	?>
	<h1>Existing Mail Pofiles</h1>
	...
	if($profiles){
		foreach($profiles as $v){
			?><tr>
			<td><a href="mail.php?Profiles_ID=<?php echo $v['ID']?>" onclick="return ow(this.href,'l1_mail','800,600');" title="open this mail profile"><?php echo $v['Name']?></a>
			</td>
			<td>&nbsp;</td>
			</tr><?php
		}	
	}else{
		?><tr><td colspan="100%">(No profiles created yet)</td></tr><?php
	}
	?>
	</tbody>
	</table>
