...
$navObject='Issues_ID';
$updateMode='updateIssue';
$insertMode='insertIssue';
$deleteMode='deleteIssue';
...
$ids=q("SELECT ID FROM per_issues WHERE 1 ORDER BY Category, CreateDate",O_COL);
...
if(strlen($$object)){
	...
	if($a=q("SELECT * FROM per_issues WHERE ID='".$$object."'",O_ROW)){
		$mode=$updateMode;
		@extract($a);
	}else{
...
<title>Restrictions<?php echo $Restrictions ? ': '.$Restrictions : ''?></title>
<!-- InstanceEndEditable -->
...
var customDeleteHandler='deleteIssue()'; //optional; default submits to bais_01_exe.php?mode=(deleteMode)
function deleteIssue(){
	//write custom delete mode here
	if(!confirm('Are you sure'))return;
	window.open('resources/bais_01_exe.php?mode=<?php echo $deleteMode;?>&deleteMode=<?php echo $deleteMode;?>&<?php echo $object;?>=<?php echo $$object;?>','w2');
}
...
if($mode==$updateMode){
	?>
	Issue created: 
	<strong><?php echo date('n/j/Y',strtotime($CreateDate));?></strong>
	<br />
	<?php
	if(strtotime($EditDate) - strtotime($CreateDate) > 15){
		?>
		Last updated: 
		<strong><?php echo date('n/j/Y \a\t g:iA',strtotime($EditDate));?></strong>
		<br />
		<?php
	}
}
?>
<p>Category: 
  <input name="Category" type="text" id="Category" value="<?php echo h($Category);?>" onchange="dChge(this);" />
  <br />
  Subcategory:
  <input name="SubCategory" type="text" id="SubCategory" value="<?php echo h($SubCategory);?>" onchange="dChge(this);" />
  </p>
<p>Restriction Description: <br />
  <textarea name="Restriction" cols="55" rows="2" id="Restriction"><?php echo h($Restriction);?></textarea>
</p>
<?php
//------- tabs coding --------
$tabPrefix='issuesMain';
$tabGroup='issuesMain';
$__tabs__[$tabGroup]['tabSet']=array(
	'Ideal'			=>'imIdeal',
	'Solution'		=>'imSolution',
	'Timeline/ETC'	=>'imTimeline',
);
$tabGroupWidth=620;
...
//-------------------------------- begin tab --------------------------
?>
<textarea name="Ideal" cols="65" rows="5" id="Ideal" onchange="dChge(this);"><?php echo h($Ideal);?></textarea>
<?php
//-------------------------------- store tab --------------------------
get_contents_tabsection('imIdeal');


//-------------------------------- begin tab --------------------------
?>
<textarea name="Solution" cols="65" rows="5" id="Solution" onchange="dChge(this);"><?php echo h($Solution);?></textarea>
<?php
//-------------------------------- store tab --------------------------
get_contents_tabsection('imSolution');


//-------------------------------- begin tab --------------------------
?>
<textarea name="Timeline" cols="65" rows="5" id="Timeline" onchange="dChge(this);"><?php echo h($Timeline);?></textarea>
<?php
//-------------------------------- store tab --------------------------
get_contents_tabsection('imTimeline');
