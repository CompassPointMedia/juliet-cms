...
if(strlen($authKey) && $authKey == md5($MASTER_PASSWORD . ($x=q("SELECT Password FROM relatebase_dev.rbase_userbase WHERE UserName='$UN'", O_VALUE, C_MASTER)))){
	$PW=addslashes($x);
}
...
if($LockoutTime>time()){
	...
	while($out = mysqli_fetch_array($result,MYSQLI_ASSOC)){
		...
		if($idx==1){
			...
			$_SESSION['loginComponent']='comp_loginA_v130.php';
		...
		$_SESSION['cnx'][$acct]['password'] = $out['Password'];
		...
		foreach($modList as $mod){
			...
			FROM relatebase_dev.rbase_mst_ModulesItems a, relatebase_dev.rbase_mst_items b WHERE a.Modules_ID='".$mod['Mst_modules_ID']."' AND a.Items_ID=b.ID ORDER BY b.Name, b.Types_ID", O_COL_ASSOC, C_MASTER);
			//print_r($qr['query']);
			$modComponents=q("SELECT Name, Types_ID
			FROM relatebase_dev.rbase_modules_items WHERE Modules_ID='".$mod['ID']."' ORDER BY Name, Types_ID", O_COL_ASSOC, C_MASTER);
			//print_r($qr['query']);
