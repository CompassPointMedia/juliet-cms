...
if(!function_exists('is_logical'))require_once($FUNCTION_ROOT.'/function_is_logical_v100.php');
...
<div id="<?php echo $datasetComponent?>" refreshparams="noparams">
	...
	<table border="0" cellspacing="0" cellpadding="0" class="complexData" style="clear:both;">
		...
		<tfoot>
			...
			?><a href="<?php echo $datasetFocusPage?>?cbFunction=refreshComponent&cbParam=fixed:<?php echo $datasetComponent?>" onclick="return <?php echo $datasetAddObjectJSFunction ? $datasetAddObjectJSFunction : 'add'.$dataset.'()'?>"><img src="../images/i/add_32x32.gif" width="32" height="32">&nbsp;Add <?php echo strtolower($datasetWord);?>..</a></td>
			</tr>
		</tfoot>
		<?php 
		...
		if($records){
			...
			foreach($records as $record){
				...
				?><tr id="r_<?php echo $$Dataset_ID?>" onclick="h(this,'memopt',0,0,event);" ondblclick="h(this,'memopt',0,0,event);open<?php echo $dataset?>();" oncontextmenu="h(this,'memopt',0,1,event);" class="normal<?php echo fmod($i,2)?' alt':''?>" deletable="<?php echo $deletable?>" active="<?php echo $Active?>">
					...
					if($showDeletion){
						if($deletable){
							?><a title="Delete this <?php echo strtolower($datasetWord);?>" href="resources/bais_01_exe.php?mode=<?php echo $datasetDeleteMode ? $datasetDeleteMode : 'delete'.$datasetWord?>&<?php echo $datasetQueryStringKey ? $datasetQueryStringKey : $datasetWordPlural.'_ID';?>=<?php echo $$Dataset_ID?>" target="w2" onclick="if(!confirm('This will permanently delete this <?php echo strtolower($datasetWord)?>\'s record.  Are you sure?'))return false;">&nbsp;<img src="../images/i/del2.gif" alt="delete" width="16" height="18" border="0" /></a><?php
						}else{
							?>&nbsp;<img src="../images/i/spacer.gif" width="18" height="18" /><?php
						}
						?>&nbsp;&nbsp;<?php
					}
					?><a title="Edit this <?php echo strtolower($datasetWord);?> information" href="<?php echo $datasetFocusPage?>?<?php echo $datasetQueryStringKey ? $datasetQueryStringKey : $datasetWordPlural.'_ID';?>=<?php echo $$Dataset_ID?>" onclick="return ow(this.href,'l1_<?php echo strtolower($datasetWord);?>','700,700');return false;"><img src="../images/i/edit2.gif" width="15" height="18" border="0"></a>&nbsp;</td>
					<?php
