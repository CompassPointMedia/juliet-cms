...
//######################### NOTE!!!! #########################
//
//		this script is also found on simple fostercare 
...
if(($m=$_COOKIE['m']) && ($Machines_ID=q("SELECT ID FROM system_machines WHERE UniqueKey='$m'", O_VALUE))){
	...
	if(!$_COOKIE['m']){
		...
		setcookie('m',$m,time()+(24*3600*180),'/','.'.$GLOBALS['HTTP_HOST']);
	}
	if($Machines_ID=q("SELECT ID FROM system_machines WHERE UniqueKey='$m'", O_VALUE)){
	...
	}else if($Machines_ID=q("SELECT ID FROM system_machines WHERE UserAgent='".addslashes($HTTP_USER_AGENT)."' AND IPAddress='".$REMOTE_ADDR."' AND MonitorResolution='".$environment."'", O_VALUE)){
		//OK
		...
		UserAgent='".addslashes($HTTP_USER_AGENT)."',
...
if($userMethod=='contacts'){
	q("REPLACE INTO system_ContactsMachines SET Contacts_ID='".$_SESSION['cnx'][$acct]['primaryKeyValue']."', Machines_ID=$Machines_ID, Comments='SQL REPLACE; login/machine_identification.php line ".__LINE__."'");
}else{
	q("REPLACE INTO bais_UniversalMachines SET ma_unusername='".$_SESSION['admin']['userName']."', Machines_ID=$Machines_ID, Comments='SQL REPLACE; login/machine_identification.php line ".__LINE__."'");
}
