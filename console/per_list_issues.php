...
?><!-- #EndLibraryItem --><!-- InstanceEndEditable -->
	<!-- InstanceBeginEditable name="top_region" -->

<!-- InstanceEndEditable -->
	<div id="leftInset">
	<!-- InstanceBeginEditable name="left_inset" -->
...
<!-- InstanceEndEditable -->
	...
	<h1>Additional Resources &gt; Issues Log </h1>
	
	<table id="issueList" class="yat">
	<thead>
	<tr>
		<th>&nbsp;</th>
		<th>Date</th>
		<th>Category</th>
		<th title="area I see that is not right or holding me back (slavery)">Problem/Restriction</th>
		<th title="what I understand the ideal should be like (freedom)">Ideal</th>
		<th title="what I propose to do to solve this">Solution</th>
		<th title="time-specific detaisls on the solution of this issue">Timeline/ETC</th>
		<th>Last Updated</th>
	</tr>
	</thead>
	<?php
	...
	<tbody>
	<?php
	if($a=q("SELECT * FROM per_issues WHERE 1 ORDER BY Category, CreateDate", O_ARRAY)){
		$i=0;
		foreach($a as $v){
			$i++;
			extract($v);
			if($i==1 || $buffer !==$v['Category']){
				$j=0;
				$buffer=$v['Category'];
				?><tr>
			...
			?><tr class="<?php echo !fmod($j,2)?'alt':''?>">
				<td><a href="per_issues.php?Issues_ID=<?php echo $ID?>" title="View/edit this solution" onclick="return ow(this.href,'l1_issues','700,700');"><img src="/images/i/edit2.gif" /></a></td>
				<td><?php echo date('n/j/Y',strtotime($CreateDate));?></td>
				<td><?php echo $Category.':'.$Subcategory;?></td>
				<td><?php echo $Restriction;?></td>
				<td><?php echo $Ideal;?></td>
				<td><?php echo $Solution;?></td>
				<td><?php echo $Timeline;?></td>
				<td><?php echo $CreateDate!=$EditDate ? date('n/j/Y \a\t g:iA',strtotime($EditDate)) : '&nbsp;';?></td>
			</tr><?php
		}
	}else{
		?><tr>
	...
	if(true){
		...
		<tfoot>
		<tr>
			<td colspan="100%"><a href="per_issues.php" title="Add a new issue/solution" onclick="return ow(this.href,'l1_issues','700,700');">Create new solution</a></td>
		</tr>
		</tfoot>
		<?php
	}
