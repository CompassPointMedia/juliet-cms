...
<form action="/console/resources/bais_01_exe.php" method="post" name="form1" target="w2" id="form1">
	...
	<h2>User Login, Account Creation and Modification Settings</h2>
	<p>This began development 9/20/2012.</p>
	<p><a title="view the &quot;Usemod&quot; settings and modify values" href="cgi.php" onclick="return ow(this.href,'l1_usemod','800,700');">User Mod. Settings Page</a></p>
	<h2>View and Layout</h2>
	<p>Word for Customer: 
		<input name="settings[ClientWord]" type="text" id="settings[ClientWord]" value="<?php echo $settings['ClientWord']?$settings['ClientWord']:'Customer';?>" onchange="dChge(this);" /> 
		e.g. Client, Customer, Dealer<br />
		Word for Item: 
		<input name="settings[ItemWord]" type="text" id="settings[ItemWord]" value="<?php echo $settings['ItemWord']?$settings['ItemWord']:'Item';?>" onchange="dChge(this);" />
	e.g. Item, Product, Part </p>
	...
	Modify Orders: 
	<select name="adminControls[modifyOrders]" id="adminControls[modifyOrders]" onchange="dChge(this);">
		<option <?php echo $adminControls['modifyOrders']=='0'?'selected':''?> value="0">Not allowed</option>
		<option <?php echo $adminControls['modifyOrders']=='1'?'selected':''?> value="1">Controls only (to export, to print)</option>
		<option <?php echo $adminControls['modifyOrders']=='2'?'selected':''?> value="2">Header editable</option>
		<option <?php echo $adminControls['modifyOrders']=='4'?'selected':''?> value="4">Shipping editable</option>
		<option <?php echo $adminControls['modifyOrders']=='8'?'selected':''?> value="8">Full order editable</option>
	</select> Each increasing value adds more permissions
	<br />
	<br />
	<?php require('components/addon.addon_masterlist_v100.php');?>

	<input type="submit" name="Submit" value="Submit" />
	<input name="mode" type="hidden" id="mode" value="updateSettings" />
</form>
<!-- InstanceEndEditable -->
	<div class="cbsm"> </div>
	</div>
