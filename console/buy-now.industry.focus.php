...
$object='Industries_ID';
$recordPKField='ID'; //primary key field
$navObject='Industries_ID';
$updateMode='updateIndustry';
$insertMode='insertIndustry';
$deleteMode='deleteIndustry';
...
$ids=q("SELECT ID FROM rv_industries WHERE 1",O_COL);
/*
...
if(strlen($$object) /* || $Assns_ID=q("SELECT ID FROM sma_assns WHERE ResourceToken!='' AND ResourceToken='$ResourceToken' AND ResourceType IS NOT NULL", O_VALUE)*/){
	...
	if($record=q("SELECT * FROM rv_industries a WHERE a.ID='".$$object."'",O_ROW)){
		$mode=$updateMode;
		@extract($record);
	}else{
...
<title><?php echo $titleBase='Industry Management '.($mode==$insertMode ? ' - Adding New Industry':'')?></title>
<!-- InstanceEndEditable -->
...
$(document).ready(function(){
	$('#OK').click(function(){ g('subsubmode').value='updateIndustry'; });
...
<div id="mainBody"><!-- InstanceBeginEditable name="main_body" -->
  <div class="optionBox1">
    Industry name:
      <input name="Name" type="text" id="Name" onchange="dChge(this);" value="<?php echo h($Name);?>" />
      <br />
      Description:<br />
      <textarea name="Description" id="Description" rows="3" cols="55" onchange="dChge(this);"><?php echo h($Description);?></textarea>
      <br />
      SIC Code (optional):
      <input name="SICCode" type="text" id="SICCode" onchange="dChge(this);" value="<?php echo h($SICCode);?>" />
      <br />
</div>
<!-- InstanceEndEditable --></div>
