	...
	FROM system_tables t, system_profiles p WHERE t.ID=p.tables_ID ORDER BY p.Identifier, t.Name", O_ARRAY)){
		...
		foreach($a as $n=>$v){
			...
			?><tr class="<?php echo !fmod($j,2)?'alt':''?>">
				<td><a href="systementry.php?object=system_tables&identifier=default&system_tables_ID=<?php echo $ID;?>" title="Edit the table configuration" onclick="return ow(this.href,'l1_tables','700,700');"><img src="/images/i/note01.gif" width="8" height="10" alt="edit" /></a>&nbsp;<a href="systementry.php?object=system_profiles&identifier=default&system_profiles_ID=<?php echo $Profiles_ID;?>" title="Edit this profile" onclick="return ow(this.href,'l1_profiles','700,700');"><?php echo $v['Name'];?></a><?php
				...
				?></td>
				<td><a href="root_systementry_list.php?_Profiles_ID_=<?php echo $Profiles_ID;?>" target="_<?php echo str_replace('_','',$SystemName);?>">open list</a> &nbsp;&nbsp;&nbsp;<a href="systementry.php?object=<?php echo $SystemName;?>&identifier=<?php echo $Identifier;?>" onclick="return ow(this.href,'l1_<?php echo $SystemName;?>','700,700'<?php
