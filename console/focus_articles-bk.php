...
Type of Content: <select name="Category" id="Category" onchange="dChge(this)">
	...
	foreach($a as $n=>$v){
		...
		?><option value="<?php echo h($key);?>" <?php echo $key==$Category?'selected':''?>><?php echo h($v);?></option><?php
	}
	if(!$haveSelected && strlen($Category)){
		?><option value="<?php echo h($Category);?>" selected="selected"><?php echo h($Category)?></option><?php
	}
	?>
</select><br />
...
foreach($contacts as $n=>$v){
	?><option value="<?php echo $n?>" <?php	if($n==$Contacts_ID)echo 'selected';?>><?php echo $v['LastName'] .', '. $v['FirstName'].($v['ClientName'] && strtolower($v['ClientName'])!==strtolower($v['FirstName'] . ' ' . $v['LastName']) ? ', '.$v['ClientName'] : '');?></option><?php
}
?>
...
<br />
Title:
<input name="Title" type="text" id="Title" onchange="mgeChge(this)" value="<?php echo $Title?>" size="60" onblur="formAction('prettyURL');" />
<br />
