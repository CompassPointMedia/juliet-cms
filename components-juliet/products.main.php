...
categoryPath, categoryBackupPath, categoryCols, 

*/

//main category image location
if(!is_dir($_SERVER['DOCUMENT_ROOT'].'/images/'.$categoryPath)){
	$a=explode('/',trim($categoryPath,'/'));
...
if(!$categoryImgs)$categoryImgs=get_file_assets($_SERVER['DOCUMENT_ROOT'].'/images/'.$categoryPath, array('positiveFilters'=>'\.(gif|jpg|png|svg)$'));
//layout
if(!$cols)$cols=$categoryCols;
if(!$rows)$rows=10000;
...
if($records){
	...
	while(list(,$v)=each($records)){
		...
		if($img=get_image($v['Category'],$categoryImgs)){
			...
			foreach($productSKUs as $w){
				...
				if($img=get_image(strtolower($w), $categoryBackupImgs)){
					...
					ob_start();
					$result=copy($_SERVER['DOCUMENT_ROOT'].'/images/'.$categoryBackupPath.'/'.$img['name'], $_SERVER['DOCUMENT_ROOT'].'/images/'.$categoryPath.'/'.$haveImage);
					$err=ob_get_contents();
					ob_end_clean();
					if($err){
						$email='Hi there,
						
						For category '.($v['Category']).', I was unable to copy picture /images/'.$categoryBackupPath.'/'.$img['name'].' to /images/'.$categoryPath.'/'.$haveImage.', here is the reply from the system:' . "\n". $err;
						$email.='
						...
						'.rtrim($siteURL,'/').'/admin/file_explorer/?uid=categoryfolder&folder='.$categoryPath.'&createFolder=1
		...
		if($haveImage){
			$Tree_ID=tree_build_path('images/'.$categoryPath.'/'.$haveImage);
