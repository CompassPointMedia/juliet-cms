...
if($thissection){
	...
	if($Parameters=q("SELECT Settings FROM gen_nodes_settings WHERE Nodes_ID='".($_thisnode_ ? $_thisnode_ : $thisnode) ."'", O_VALUE)){
		...
		//----------------------------------------------------------------------------		
		/*
		on 2012-11-29 I fought a vicious and brutal battle with PHP.. I lost this and am disgusted at what happened.
		*/
		//store what session WAS like
		$temp=$_SESSION['pJ'];
		
		#prn($temp);
		
		//add a new LOCAL node without mentioning session
		#$_SESSION['inc']++;
		#$pJ['newnode '.$_SESSION['inc']]=1;
		if($Parameters[$handle])$pJ['componentFiles'][$handle]=$Parameters[$handle];
		
		//get ready for it..
		#prn($_SESSION['pJ']);
		
		//so I have been screwed by PHP..
		
		//now do something about it
		foreach($_SESSION['pJ'] as $n=>$v){
			if(!isset($temp[$n])){
				#prn('this node '.$n.' was declared locally but wound up in session');
				#prn('unsetting it!');
				#unset($_SESSION['pJ'][$n]);
			}
		}
		//big question.. did it work?
		#prn($_SESSION['pJ']);
		
		//but what about the global level?
		#prn($GLOBALS['pJ']);
		
		//yeah, that got munched too.. PHP=SOB
		//----------------------------------------------------------------------------		
